    ...
    var methods = {
        init: function (options) {
            this.each(function () {
                ...
                if ($scrollObj.attr('onscroll')) {
                    var objOnscroll = $scrollObj.attr('onscroll');
                    var mousewheelevt = (/Firefox/i.test(navigator.userAgent)) ? "DOMMouseScroll" : "mousewheel" //FF doesn't recognize mousewheel as of FF3.x
                    if (document.attachEvent) //if IE (and Opera depending on user setting)
                    {
                        if (objOnscroll) {
                            $scrollObj.unbind("on" + mousewheelevt);
                            $scrollObj.bind("on" + mousewheelevt, function (e) {
                                objOnscroll.call();
                            });
                        }
                    }
                    ...
                    {
                        $scrollObj.unbind(mousewheelevt);
                        $scrollObj.bind(mousewheelevt, function (e) {
                            if (objOnscroll)
                                objOnscroll.call();
                        });
                    }
                }
                //                if ($(this).attr('onscroll'))
                //                    $scrollObj.wrapInner('<div class="alt-scroll-holder" style="position: relative;" onscroll="' + $(this).attr('onscroll') + '><div class="alt-scroll-content" style="position: absolute; top: 0; left: 0"></div></div>');
                //                else
                ...
                function mouseWheel(init) {
                    ...
                    if (init) {
                        if (window.addEventListener) {
                            if (document.attachEvent) //if IE (and Opera depending on user setting)
                                window.attachEvent("on" + mousewheelevt, handleMouseWheel, false);
                            else if (document.addEventListener) //WC3 browsers
                                window.addEventListener(mousewheelevt, handleMouseWheel, false);
                        ...
                        if (window.removeEventListener) {
                            if (document.attachEvent) //if IE (and Opera depending on user setting)
                                window.removeEventListener("on" + mousewheelevt, handleMouseWheel);
                            else if (document.addEventListener) //WC3 browsers
                                window.removeEventListener(mousewheelevt, handleMouseWheel);
                        }
                ...
                function handleMouseWheel(event) {
                    ...
                    //Enable this section if you want to scroll document windo in case of MAX H or V scroll
                    try {
                        if (newContentTop == 0) {
                            var newDocTopScroll = $(window.parent, window.parent.document).scrollTop();
                            $(window.parent, window.parent.document).scrollTop(newDocTopScroll - 50);
                            //$("html, body").animate({ scrollTop: newDocTopScroll - 50 }, "fast");
                        }
                        else if ((scrollHolderHeight - scrollContentHeight) == newContentTop) {
                            var newDocTopScroll = $(window.parent, window.parent.document).scrollTop();
                            $(window.parent, window.parent.document).scrollTop(newDocTopScroll + 50);
                            //$("html, body").animate({ scrollTop: newDocTopScroll + 50 }, "fast");
                        }
